 ...
 *
 * None of the tests below stub `/registration/user/is-migrateable` —
 * `onSubmit` awaits it before ever calling `doLogin`, so they rely on the
 * real backend responding "not migratable, not excluded" for these
 * throwaway e2e credentials. The conditions where that response actually
 * matters (self-excluded, migratable) are covered in their own isolated
 * spec: `cypress/e2e/legacy/login/self-excluded.cy.ts`
 * (`pnpm cypress:run:legacy:self-excluded`).
 */
